...
// Rute untuk halaman berita cuaca berdasarkan lokasi
app.get('/berita', (req, res) => {
    const lokasi = req.query.lokasi;

    if (!lokasi) {
        return res.render('berita', {
            judul: 'Berita Terkini Seputar Cuaca',
            error: 'Silakan masukkan lokasi untuk melihat berita cuaca.'
        });
    }

    geocode(lokasi, (error, { latitude, longitude, location } = {}) => {
        if (error) {
            return res.render('berita', {
                judul: 'Berita Terkini Seputar Cuaca',
                error: 'Lokasi tidak ditemukan. Silakan coba lokasi lain.'
            });
        }
        ...
        forecast(latitude, longitude, (error, dataPrediksi) => {
            if (error) {
                return res.render('berita', {
                    judul: 'Berita Terkini Seputar Cuaca',
                    error: 'Tidak dapat mengambil data cuaca saat ini.'
                });
            }
